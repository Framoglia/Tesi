...
for district in set(b.district for b in LBUS.values() if b.district and b.district != 'TFO'):
    ...
    while True:   
        ...
        if count != 0:
            ...
            #cond_table = reset_conductor_table(cond_table, LINES, LINES_OPT)
        ...
        try:
            ...
            powerflow_results.append(results)
            voltage_df , loading_df = pf_hm(results, pp_bus_map, pp_line_map, count)
            models.append(model)
            
        except TypeError as e:
            print(f"Error in optimization for district {district}: {e}")
            break  
