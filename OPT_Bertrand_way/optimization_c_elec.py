...
def optimize_log(LBUS, SUBS, SLACK, LINES, LINES_OPT, N_PERIODS, irradiation, weights, initial_config, setting = [2,15,1,16]):
    ...
    model.C_electricity = Var(model.periods, model.buses, within=Reals)
    model.C_elec_yearly = Var(model.buses, within=Reals) # Reals => can compensate for other buses deficit
    model.C_losses = Var(model.periods, within=NonNegativeReals)  
    model.unserved_fictitious_power_cost = Var(within=NonNegativeReals)

    model.total_C_electricity = Expression(expr=sum(model.C_elec_yearly[b] for b in model.buses))
    ...
    def energy_imp_cost_rule(m,p,b): ##Why should i pay P bus and not S_HV  it would already count losses??
        return m.C_electricity[p,b] >= m.P_bus[p,b] * BASE_POWER / 1000 * ENERGY_COST_IMP * DELTA_T
    
    def energy_exp_cost_rule(m,p,b):
        return m.C_electricity[p,b] >= m.P_bus[p,b] * BASE_POWER / 1000 * ENERGY_COST_EXP * DELTA_T
    ...
    def storage_energy_rule(m,p,b):
        if (p-1)%24 != 0 and (p-1)%24 != 1:
            ...
            return m.storage_energy[p,b] == m.storage_energy[p-1,b] + ((m.P_storage_charge[p,b] + m.P_storage_charge[p-1,b]) * STORAGE_EFFICIENCY - (m.P_storage_discharge[p,b] + m.P_storage_discharge[p-1,b]) / STORAGE_EFFICIENCY) * DELTA_T
        else:
    ...
    def yearly_cost_electricity_rule(m,b):
        return m.C_elec_yearly[b] == sum(model.C_electricity[p,b] * get_weights(p, weights) for p in model.periods)
    ...
    model.energy_imp_cost_cstr = Constraint(model.periods, model.buses, rule=energy_imp_cost_rule)
    model.energy_exp_cost_cstr = Constraint(model.periods, model.buses, rule=energy_exp_cost_rule)
    ...
    model.yearly_cost_electricity_cstr = Constraint(model.buses, rule=yearly_cost_electricity_rule)




    def objective_rule(m):
        return 1/INV_HORIZON_DSO * (m.C_subs_hv + m.C_subs_mv + m.C_cond + m.C_inv + m.C_PV + m.PV_inst_cost + m.subs_hv_inst_cost + m.subs_mv_inst_cost + m.storage_inst_cost + m.C_storage_capacity) + (m.total_C_electricity + m.total_C_losses)  + OMEGA * (m.total_phi + m.unserved_fictitious_power_cost)
