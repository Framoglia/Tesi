...
def load_conductors_csv(file_path = BASE_DIR / "Campus data" / "MyCampus" / "no_conductors.csv"):
    conductors_dict = {}
...
def build_cond_table(LINES, LINES_OPT):
    cond_table = {}
    best_conductor = max(LINES_OPT,key=lambda k: LINES_OPT[k].cost_keur_per_km)
    ranked_conductors = sorted(LINES_OPT,key=lambda k: LINES_OPT[k].cost_keur_per_km,reverse=True)
    for line_id in LINES.keys():
        cond_list = {}
        for conductor_id in LINES_OPT.keys():
            if conductor_id == best_conductor:
                cond_list[conductor_id] = 1
            else:
                cond_list[conductor_id] = 0
        cond_table[line_id] = cond_list
    return cond_table, ranked_conductors

def downgrade_conductors(cond_table, ranked_conductors, lines_to_downgrade):
    """
    ...
    for line_id in lines_to_downgrade:
        ...
        # Find the index of the current conductor in the ranked list
        current_index = ranked_conductors.index(current_cond)
        
        # If not already the worst conductor (last in ranked list)
        if current_index < len(ranked_conductors) - 1:
            print(f'updated conductor of line {line_id}')
            # Get the next worse conductor
            next_worse_cond = ranked_conductors[current_index + 1]
...
def get_lines_with_upgradable_conductors(cond_table, ranked_conductors, loading_df):
    """
    Filter loading_df to only include lines that can be downgraded (not worst conductor)
    
    Args:
        cond_table: Dictionary of dictionaries showing current conductor assignments
        ranked_conductors: List of conductor IDs sorted from best to worst
        loading_df: DataFrame with line loading percentages
        
    Returns:
        Filtered DataFrame containing only lines that can be downgraded
    """
    worst_conductor = ranked_conductors[-1]
    upgradable_lines = []
    
    for line_id in loading_df.columns:
        current_cond = next(cond for cond in cond_table[line_id] if cond_table[line_id][cond] == 1)
        if current_cond != worst_conductor:
