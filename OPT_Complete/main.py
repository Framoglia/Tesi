...
centralized = group_folders(folders, group_name='Centralized')
approaches.append(centralized)
...
for district in set(b.district for b in LBUS.values() if b.district and b.district != 'TFO'):
    ...
    while True:   
        ...
        model, logg = optimize_dnep(new_LBUS_d, SUBS_d, SLACK, LINES, LINES_OPT, N_PERIODS, new_irradiation, weights, initial_config, cond_table)
        models.append(model)
        try:
            plot_opt(model, new_LBUS_d, SUBS_d, SLACK, LINES, LINES_OPT, N_PERIODS)
            export_optimal_values(model, count)
...
plot_opt_district(district_results, SLACK, LINES_OPT)
bilevel = group_folders(folders, group_name='Bilevel')
approaches.append(bilevel)
organization = group_folders(approaches, group_name="Double_Brussels")
