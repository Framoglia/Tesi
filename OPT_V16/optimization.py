...
def optimize_log(LBUS, SUBS, SLACK, LINES, LINES_OPT, N_PERIODS, irradiation, EV_option, weights, initial_config, stage, setting = [2,15,1,16]):
    ...
    def loss_cost(m,p):
        return m.C_losses[p] * SCALING == sum(LINES_OPT[c].r_per_km / fetch_base_z_from_line(DATA, l) * LINES[l].length  / 100 * m.current_squared_k[p,l,c] * SCALING for l in m.lines for c in m.conductors) * BASE_POWER / 1000 * UNIT_COST_LOSSES * DELTA_T
    ...
    def beta_switch_fict_rule(m,s):
        return m.beta[s] * N_PERIODS * len(LBUS.keys()) >= sum(m.subs_hv_F[p,s] for p in m.periods) 
    ...
    def voltage_rule_1(m,p,l):
        ...
        return (m.voltage_squared[p,j] - m.voltage_squared[p,i]) / scaling_factor * SCALING / 10 <= sum(-2 * (LINES_OPT[c].r_per_km * m.active_power_k[p,l,c] + LINES_OPT[c].xl_per_km * m.reactive_power_k[p,l,c]) * SCALING / 10 + m.current_squared_k[p,l,c] * SCALING / 10 * ((LINES_OPT[c].r_per_km **2 + LINES_OPT[c].xl_per_km **2) * scaling_factor) for c in m.conductors) + M * (1-m.line_act[l])
    ...
    def complex_power_rule(m,p,l):
        return  m.current_squared[p,l] * SCALING >= sum(m.SPWB[l,db] * (m.active_power_discr[p,l,db] + m.reactive_power_discr[p,l,db]) * SCALING for db in m.NPWB)
    ...
    def PV_investment_rule(m,b):
        return m.pi[b] * 1000000 >= m.PV_surf[b]
    ...
    def bus_active_power_balance_rule(m,p,b):
            if EV_option:
                return m.P_bus[p,b] == m.P_load[p,b] - m.P_sun[p,b] + m.P_storage_charge[p,b] - m.P_storage_discharge[p,b] + sum(m.EV_ch_P[p,e] - m.EV_disch_P[p,e] for e in m.lv_load if (hasattr(LBUS[e], 'vehicle_location') and LBUS[e].vehicle_location is not None and LBUS[e].vehicle_location[(p - 1) % 24] == b)) 
            else:
                return m.P_bus[p,b] == m.P_load[p,b] - m.P_sun[p,b] + m.P_storage_charge[p,b] - m.P_storage_discharge[p,b]
    ...
    def objective_rule(m):
        if EV_option:
            return 1/INV_HORIZON_DSO * (m.C_subs_hv + m.C_subs_mv + m.C_cond + m.C_inv + m.C_PV + m.PV_inst_cost + m.subs_hv_inst_cost + m.subs_mv_inst_cost + m.storage_inst_cost + m.C_storage_capacity + m.C_storage_power + m.C_EV) + (m.total_C_electricity + m.total_C_losses + m.total_C_gas)  + OMEGA * (sum(m.phi[p] * get_weights(p, weights) for p in m.periods) + m.unserved_fictitious_power_cost)
        else:
             return 1/INV_HORIZON_DSO * (m.C_subs_hv + m.C_subs_mv + m.C_cond + m.C_inv + m.C_PV + m.PV_inst_cost + m.subs_hv_inst_cost + m.subs_mv_inst_cost + m.storage_inst_cost + m.C_storage_capacity + m.C_storage_power) + (m.total_C_electricity + m.total_C_losses)  + OMEGA * (sum(m.phi[p] * get_weights(p, weights) for p in m.periods) + m.unserved_fictitious_power_cost)
