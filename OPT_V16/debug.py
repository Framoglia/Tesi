...
def plot_topology_basic(LBUS, SUBS, SLACK, LINES):
    ...
    for BUS_SET in [LBUS, SUBS, SLACK]:
        for bus in BUS_SET:
            x_max = max(x_max, BUS_SET[bus].x_coord)
            y_max = max(y_max, BUS_SET[bus].y_coord)
            x_min = min(x_min, BUS_SET[bus].x_coord)
            y_min = min(y_min, BUS_SET[bus].y_coord)
...
def group_folders(folders, group_name='all_organized_folders'):
    ...
    for folder in folders:
        if os.path.exists(folder):
            shutil.move(folder, os.path.join(group_name, folder))
    print(f"Grouped all folders into '{group_name}'.")
    ...
    for t in model.periods:
        ...
        for bus_id in lbus_ids:
            ...
            if hasattr(model, "storage_capacity") and model.storage_capacity[bus_id].value > 0.0005:
                ...
                storage_transf = model.P_storage_charge[t, bus_id].value - model.P_storage_discharge[t, bus_id].value
                storage_soc = model.storage_energy[t, bus_id].value / model.storage_capacity[bus_id].value *100
                updated_storage_text.append(
                    f"Storage on Bus {bus_id}<br>Capacity: {storage_capacity:.3f}<br>Maximum power: {storage_power:.3f}<br>Power (charge): {storage_transf:.3f}<br>SOC: {storage_soc:.1f}"
                )
            else:
                updated_storage_text.append("")
        ...
        for sub_id, sub in SUBS.items():
            if hasattr(model, "gamma"):
                if model.gamma[sub_id].value >= 0.8:
                    ...
                    sub_power = (model.subs_mv_P[t,sub_id].value**2+model.subs_mv_Q[t,sub_id].value**2)**0.5
