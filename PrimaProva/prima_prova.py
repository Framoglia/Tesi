...
model.p_exp = Param(model.periods, model.buses, mutable=True)
model.q_imp = Param(model.periods, model.buses, mutable=True)
model.q_exp = Param(model.periods, model.buses, mutable=True)

#Parameters (this will come from the user sizing)
if len(model.periods) == 1:
    p = 1
    for b in model.buses:  
        if LBUS[b].load_MW > 0:
            model.p_imp[p,b] = (LBUS[b].load_MW)
            model.p_exp[p,b] = 0
        else:
            model.p_exp[p,b] = (LBUS[b].load_MW)
            model.p_imp[p,b] = (0)

        if LBUS[b].load_MVAR > 0:
            model.q_imp[p,b] = (LBUS[b].load_MVAR)
            model.q_exp[p,b] = (0)
        else:
            model.q_exp[p,b] = (LBUS[b].load_MVAR)
            model.q_imp[p,b] = (0)

else:
    for p in model.periods: 
        for b in model.buses:  
            if LBUS[b].load_MW[p-1] > 0:
                model.p_imp[p,b] = (LBUS[b].load_MW[p-1])
                model.p_exp[p,b] = 0
            else:
                model.p_exp[p,b] = (LBUS[b].load_MW[p-1])
                model.p_imp[p,b] = (0)

            if LBUS[b].load_MVAR[p-1] > 0:
                model.q_imp[p,b] = (LBUS[b].load_MVAR[p-1])
                model.q_exp[p,b] = (0)
            else:
                model.q_exp[p,b] = (LBUS[b].load_MVAR[p-1])
                model.q_imp[p,b] = (0)

print(INV_HORIZON_DSO * ALPHA * (sum(model.p_imp[p,b].value for b in model.buses for p in model.periods)*ENERGY_COST))
...
def budget_balance(m):
    return (1+DISCOUNT_RATE)**INV_HORIZON_DSO * (m.C_cond + m.C_subs) + INV_HORIZON_DSO * ALPHA *sum(m.C_losses[p] for p in m.periods) <= INV_HORIZON_DSO * ALPHA * (sum(m.p_imp[p,b] for b in m.buses for p in m.periods)*ENERGY_COST - sum(m.p_exp[p,b] for b in m.buses for p in m.periods)*FEED_IN_TARIFF) * DELTA_T
...
def active_power_lbus_rule(m,p,b):
    return m.p_imp[p,b] - m.p_exp[p,b] == (sum(m.active_power_k[p,l,c] - m.current_squared_k[p,l,c] * LINES_OPT[c].r_per_km * LINES[l].length for c in m.conductors for l in m.lines if LINES[l].to_bus==b) - sum(m.active_power_k[p,l,c] for c in m.conductors for l in m.lines if LINES[l].from_bus==b))

def reactive_power_lbus_rule(m,p,b):
    return m.q_imp[p,b] - m.q_exp[p,b] == (sum(m.reactive_power_k[p,l,c] - m.current_squared_k[p,l,c] * LINES_OPT[c].xl_per_km * LINES[l].length for c in m.conductors for l in m.lines if LINES[l].to_bus==b) - sum(m.reactive_power_k[p,l,c] for c in m.conductors for l in m.lines if LINES[l].from_bus==b))
