...
def optimize_log(LBUS, SUBS, SLACK, LINES, LINES_OPT, N_PERIODS, irradiation, weights, setting = [1,15,1,16]):
    ...
    def sos1_line_act_rule(m, l):
        return [m.line_act_plus[l], m.line_act_minus[l]]
    ...
    def storage_energy_rule(m,p,b):
        if (p-1)%24 != 0:
            ...
            return m.storage_energy[p,b] == m.storage_energy[p+23,b]
    ...
    model.sos1_line_act = SOSConstraint(model.lines, rule=sos1_line_act_rule, sos=1)
    ...
    def objective_rule(m):
        return 1/INV_HORIZON_DSO * (m.C_subs + m.C_cond + m.C_inv + m.C_PV + m.PV_inst_cost + m.subs_hv_inst_cost + m.subs_mv_inst_cost + sum(m.C_storage[b] for b in model.buses)) + sum((m.C_electricity[p] + m.C_losses[p]) * get_weights(p, weights) + OMEGA * m.phi[p] for p in m.periods) + OMEGA * m.unserved_fictitious_power_cost
