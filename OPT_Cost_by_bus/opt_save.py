...
def optimize_log(LBUS, SUBS, SLACK, LINES, LINES_OPT, N_PERIODS, irradiation, EV_option, weights, initial_config, stage, setting = [2,15,1,16]):
    ...
    def pv_production_rule(m,p,b):
        return m.P_pv[p,b] * BASE_POWER <= m.PV_surf[b] * m.Irr[p] * 0.2
        ...
        if EV_option:
            return 1/INV_HORIZON_DSO * (m.C_subs_hv + m.C_subs_mv + m.C_cond + m.C_inv + m.C_PV + m.PV_inst_cost + m.subs_hv_inst_cost + m.subs_mv_inst_cost + m.storage_inst_cost + m.C_storage_capacity + m.C_storage_power + m.C_EV) + (m.total_C_electricity + m.total_C_losses + m.total_C_gas)  + OMEGA * (sum(m.phi[p] * get_weights(p, weights) for p in m.periods) + m.unserved_fictitious_power_cost)
             ...
             return 1/INV_HORIZON_DSO * (m.C_subs_hv + m.C_subs_mv + m.C_cond + m.C_inv + m.C_PV + m.PV_inst_cost + m.subs_hv_inst_cost + m.subs_mv_inst_cost + m.storage_inst_cost + m.C_storage_capacity 
                                         #+ m.C_storage_power
                                         ) + (m.total_C_electricity + m.total_C_losses)  + OMEGA * (sum(m.phi[p] * get_weights(p, weights) for p in m.periods) + m.unserved_fictitious_power_cost)
    ...
    solver.options['TimeLimit'] = 3600 * 14
